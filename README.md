...
|-|-|
| This repository contains code for parsing *dose instructions*. These are short pieces of free text written on prescriptions to tell patients how to use their medication. An example prescription is shown to the left, with the dose instruction "125mg three times daily" highlighted. The code can be used to parse this dose instruction from the command line in the following way: | !["Example prescription with dose instruction '125mg three times daily' source: BNF" ](./doc/sphinx/source/_static/bnf_prescription_example.png) |
